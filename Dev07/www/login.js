...
{
    $('[lbl="welcome"]').on({click:function() {
        $('[lbl="welcome"]').append($("<lable>").html("<br/>Server is: " + serverUrl + ""))
    } })
	...
	$("[act='ShowHideCode']").on({click:function() {
		if($("[container='initCode']").attr("closed") == "true")
		{
			$("[act='ShowHideCode']").html("<a class='btn red' href='#back'>Back to standard login <i class='fa fa-lock'></i></a>");
			$("[container='login2server']").hide('fast', function(){ $("[container='initCode']").show('fast') });
			$("[container='initCode']").attr("closed","false");
		}
		else{
			$("[act='ShowHideCode']").html("<a href='#back' class='btn red'>I have a code <i class='fa fa-lock'></i></a>");
			$("[container='initCode']").hide('fast', function(){ $("[container='login2server']").show('fast') });
    ...
    if (guestUser != "")
        $("[btn='SubmitLoginG']").show();
    $("[btn='SubmitLoginG']").on({click:function() {    
        $('[lbl="login.loading"]').html("Loading... Please wait...");
    ...
    $.ajax(  
		{   
                ...
                data: JSON.stringify(
                    { 
                        Input: { 
                            UserName: $('[txt="username"]').val(), Password: $('[txt="password"]').val() 
                        }
                    }),
...
function LOGIN_ParseCode()
{
    ...
    $('[container="footer"]').html("Checking configurations.... <br/>Please wait...");
    $.ajax(  
		{  
                type: "POST",
                dataType: 'json',
            ...
            success:function(data){ 
                ...
                if (authdata.Success) {
                    $('[lbl="authcode.loading"]').html("System is now connecting to " + authdata.ServerName);
                    serverUrl = authdata.Url;
                    $('[lbl="login.loading"]').html("System is now connecting to " + authdata.ServerName);
		    ...
		    $('[lbl="authcode.loading"]').html("Unable to login.<br/>Please ensure connectivity to the internet.");
            $('[action="ParseCode"]').removeAttr("disabled");
                // Find the offline config
        } );   
}
